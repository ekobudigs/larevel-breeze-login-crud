    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div>
                ...
                @endif
                <form action="{{ route('employee.update', $employee->id) }}" class="w-full" method="POST"
                    ...
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            ...
                            <input value="{{ old('phone') ?? $employee->phone }}" name="phone"
                                class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                                id="grid-last-name" type="number" placeholder=" phone">
                        </div>
                    </div>

                    <label for="companie_id"
                    ...
                    <select id="companie_id" name="companie_id"
                        ...
                        @foreach ($companie as $comp)
                            <option value="{{ $comp->id }}" @selected($employee->companie_id == $comp->id)>
                                {{ $comp->name }}
                            </option>
                        @endforeach
